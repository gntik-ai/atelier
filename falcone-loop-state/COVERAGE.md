...
|---|---|---|---|---|
| Identity & tenancy | 6 | 7 | 86% → **PARTIAL** | A1/A5/A6/A7 PASS live; A3 gap confirmed. A4 blocked by #953. **[DOWNGRADED 2026-08-09, triage batch]** Provisioning PASS, but the identity-claim and membership planes FAIL: ~~**#961** (no principal ever receives a `workspace_id` claim)~~ **FIXED `01e966f2`, verifier CONFIRMED-FIXED — tenant realms now declare the attributes and `workspace-context` mints the claim. **LIVE ON STAGING** since 2026-08-09 (same image, `workspace_id` present in `kc-admin.mjs`), so NEW principals get the claim. **Realm back-fill RUN 2026-08-09** (runbook §5, in-cluster Job, `--apply`): 2 of 2 active tenant realms repaired, 0 failures, and §7 verification PASSES on both — `tenant_id`/`workspace_id` declared with `permissions.edit == ["admin"]`, `workspace-context` carrying an `oidc-usermodel-attribute-mapper` for `workspace_id`, and `tenant-context` still carrying **no** user-attribute mapper (deliberate). **The §6 user re-stamp is NOT done** — 1 principal (`llmwiki-s2-e2e`) holds no stored `workspace_id` and no realm-level change can supply one; the runbook forbids bulk-assigning a default workspace because a guessed binding is a granted authorization**, **#975** (invitations write-only, never read), **#979**/**#969** (no Keycloak client is ever materialized), **#960** (plan assignment discarded on the create-tenant path). **Row stays PARTIAL**: the claim plane is repaired, the membership plane (#975 #979 #969) and #960 are not. |
