...
Every implementation and delivery task is intentionally unchecked. This artifact specifies future
work only; the spec-author phase does not implement or self-approve C-16.

## 1. Preserve the C-16 boundary

- [ ] 1.1 Re-read `proposal.md`, `design.md`, and `specs/observability/spec.md`; resolve any independent
  spec-critic blocking issue before changing product code.
- [ ] 1.2 Limit the implementation diff to the existing metrics scope guard, workspace storage-usage
  handler, unified OpenAPI and generated public-API family/catalog/reference, affected console error
  state, focused tests, detailed docs, and this OpenSpec change.
- [ ] 1.3 Confirm the change adds no role, permission, route, operation id, store, persistence schema,
  migration, domain audit event, metric family, quota/metering mutation, rate-limit rule, gateway/Helm
  configuration, production UI redesign, or remediation for C-01/C-02/C-04/C-09/C-10.
- [ ] 1.4 Keep all validation local and hermetic; do not deploy to or mutate a Kubernetes/OpenShift
...
- [ ] 2.1 Add `tests/blackbox/scoped-resource-existence-c16.test.mjs` (or the repository's focused C-16
...
- [ ] 2.2 Cover all six tenant handler families—quota posture, overview, usage snapshot, runtime-only
...
- [ ] 2.3 Extend `tests/blackbox/metrics-tenant-authorization.test.mjs` or the focused C-16 suite to prove
  authentication remains first and constrained foreign-existing and unrelated-unknown tenants both
  return indistinguishable `403 FORBIDDEN` without `getTenant`, metric attribution, registry, or
  provider work.
- [ ] 2.4 Add workspace-metrics preservation cases proving unknown workspace `404`, known foreign
  workspace `403`, authorized existing workspace success, and no separate owning-tenant registry
  re-probe.
- [ ] 2.5 Add storage-usage cases proving privileged unknown workspace `404`, constrained foreign-
...
- [ ] 3.1 In `apps/control-plane/metrics-handlers.mjs`, add exactly one
  `tenant-store.getTenant` confirmation for tenant paths after `canManageTenant` succeeds and before
  `ctx.resolvedScope`, metric attribution, parameter handling, or leaf-handler invocation.
- [ ] 3.2 Return the existing handler-level `err(404, 'TENANT_NOT_FOUND', ...)` for a null/absent tenant
  row across all six shared-guard handlers; do not swallow a datastore exception into `404` or a
  degraded `200`.
- [ ] 3.3 Preserve the tenant authorization short-circuit and the complete workspace metrics branch,
  including workspace lookup, unknown `404`, known-foreign `403`, canonical owning-tenant scope, and
  provider behavior.
- [ ] 3.4 In `apps/control-plane/storage-handlers.mjs`, resolve the workspace for every
...
- [ ] 3.5 Ensure both terminal storage `404` paths return before `listBucketsForWorkspace`,
...
- [ ] 3.6 Keep the implementation in already packaged modules with no new runtime dependency; if a
...
- [ ] 4.1 Add a JSON `404` response referencing `#/components/schemas/ErrorResponse` in
...
- [ ] 4.2 Add the same canonical response for workspace operations `getWorkspaceQuotaPosture`,
...
- [ ] 4.3 Assert that no tenant series operation is published, `getWorkspaceStorageUsage` retains its
...
- [ ] 4.4 Run `npm run generate:public-api` and inspect the generated metrics family, public route
  catalog, and public API reference for only the expected source-derived changes.
- [ ] 4.5 Add a focused contract test that discovers operations by operation id, compares the exact
  eleven-operation `404` set, validates each `$ref`, proves tenant series remains absent, preserves
  storage `404`, and fails on generated-artifact drift.
- [ ] 4.6 Exercise the resulting HTTP `404` responses against the existing C-02 closed
...
- [ ] 5.1 Update tenant metrics fixtures to create an authoritative tenant row before any test that
  intentionally exercises a real scope; remove accidental reliance on a path id with no registry row.
- [ ] 5.2 Prove an existing tenant with no configured limits or provider evidence still returns its
  schema-valid honest empty/degraded `200` for quota posture, overview, usage, and series.
- [ ] 5.3 Prove an existing tenant with no matching audit rows still returns the current schema-valid
  empty audit collection and export manifest at `200`.
- [ ] 5.4 Update storage fixtures to create an authoritative workspace row and prove a real workspace
  with no buckets or objects keeps its truthful zero-valued usage snapshot and existing quota/default
  math at `200`.
- [ ] 5.5 Retain populated tenant/workspace fixtures that compare every affected success schema,
...
- [ ] 6.1 Extend `apps/web-console/src/lib/console-quotas.test.ts` with a success-to-scope-`404`
  transition proving both tenant and workspace posture are cleared and the existing retry/error state
  is exposed.
- [ ] 6.2 Extend `apps/web-console/src/lib/console-metrics.test.ts` with success-to-`404` cases proving
  overview, usage, and series state is cleared rather than retained or normalized into a healthy empty
  result.
- [ ] 6.3 Cover audit list and export `404`: records/pagination are cleared, no empty-history success or
  export manifest is fabricated, and the existing error feedback remains available.
- [ ] 6.4 Extend `ConsoleStoragePage` tests with a workspace-selection or reload transition from a real
  usage snapshot to `404`, proving stale/zero usage is cleared and the existing unavailable/error state
  renders.
- [ ] 6.5 Make only the minimal client/hook/page state-reset correction exposed by these regressions;
...
- [ ] 7.1 Add a detailed architecture reference for scoped resource existence covering the complete
...
- [ ] 7.2 Document the local handler classes and canonical C-02 on-wire envelope, the exact eleven
...
- [ ] 7.3 Document real-empty versus absent examples for quota/usage/series/audit/export/storage,
  downstream short-circuits, read-only data-governance/telemetry/quota behavior, console stale-state
  handling, compatibility, rollback, and focused local troubleshooting/validation commands.
- [ ] 7.4 Update or cross-link the observability success, storage capacity, and generated public API
...
- [ ] 8.1 Re-run the C-16 black-box suite after the fix and verify every pre-fix red case is green,
  including call-order and zero-downstream-work assertions.
- [ ] 8.2 Run the existing tenant authorization, metrics degradation/success-schema, audit contract,
  storage quota/usage, and focused OpenAPI/public-interface regression suites affected by fixture or
  ordering changes.
- [ ] 8.3 Run the focused web-console quotas, metrics/audit, observability page, and storage page Vitest
...
- [ ] 8.4 Run `npm run generate:public-api`, verify a clean second generation, then run
  `npm run validate:public-api` and `npm run validate:openapi`.
- [ ] 8.5 Run markdownlint on all changed Markdown, applicable syntax/lint checks, and
  `git diff --check`; repair every C-16-owned failure.
- [ ] 8.6 Run `openspec validate fix-c16-scoped-resource-existence --strict` and repair every issue
  before checker handoff.
- [ ] 8.7 Confirm no cluster deployment or shared environment mutation was performed and no deployment,
...
- [ ] 9.1 Give an independent OpenSpec critic the analyst requirements, architect design, C-16 artifacts,
  and strict-validation command; resolve every blocking precision, completeness, or delta-scope issue
  before implementation begins.
- [ ] 9.2 Have an independent verifier reproduce the pre-fix finding and then confirm the fixed
...
- [ ] 9.3 Obtain independent contract and authorization/isolation verdicts for the exact eleven-operation
  contract, C-02 envelope reuse, P10/P13 boundaries, and absence of tenant enumeration.
- [ ] 9.4 Obtain independent persona/journey, console UX/accessibility, and documentation verdicts for
  P1/P3/P4/P9 primary flows and P7/P10/P12/P13 adjacent/adversarial controls, without cluster access.
- [ ] 9.5 Give a final reviewer the requirements, implementation diff, fresh commands/results, and prior
  checker verdicts; address every blocking item and rerun affected checks without maker self-approval.
- [ ] 9.6 Inspect `git status` and the staged diff; force-add only the ignored
