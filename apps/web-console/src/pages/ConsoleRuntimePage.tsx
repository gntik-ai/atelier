...
export function ConsoleRuntimePage() {
  ...
  const dependencyLabel = stateLabels[status.state] ?? 'Estado no disponible'
  return <div className="space-y-6" aria-labelledby="runtime-title">
    ...
    <section className="grid gap-4 md:grid-cols-2" aria-label="Capacidades dependientes del runtime">
      {[['Funciones', '/console/functions'], ['Hosted MCP', '/console/mcp/servers/srv_1']].map(([label, href]) => <article key={label} className="rounded-3xl border border-border bg-card p-5"><h2 className="font-semibold">{label}</h2><p className="mt-2 text-sm text-muted-foreground">{dependencyLabel}. No hay acciones de mutación desde esta vista.</p>{status.state !== 'ready' ? <Button type="button" variant="outline" className="mt-4" onClick={load} ref={retryRef}>Volver a comprobar</Button> : null}<Button asChild variant="outline" className="mt-4 ml-2"><Link to={href}>Inspeccionar {label}</Link></Button></article>)}
    </section>
  </div>
}
