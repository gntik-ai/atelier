...
| DELETE | `/v1/iam/tenant-roles/{roleId}` | tenant | iam_custom_role | Soft-delete a tenant custom RBAC role and trigger effective-permissions recalculation |
| GET | `/v1/iam/tenant-roles/{roleId}` | tenant | iam_custom_role | Fetch one tenant custom RBAC role scoped to the caller's tenant |
...
| Method | Path | Scope | Resource | Summary |
| --- | --- | --- | --- | --- |
| GET | `/v1/platform/billing/usage` | platform | billing_usage_record | List billing usage records |
| GET | `/v1/platform/billing/usage/{tenantId}` | platform | billing_usage_record | List billing usage records for one tenant |
...
| GET | `/v1/workspaces/{workspaceId}/privilege-domains/audit` | workspace | privilege_domain_denial | Query privilege-domain denial history for one workspace |
